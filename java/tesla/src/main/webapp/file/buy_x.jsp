<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>주문하기</title>
    <link rel="stylesheet" href="../css/buyy.css">
    <link rel = "stylesheet" href="../css/bootstrap.css">
    <script src ="https://code.jquery.com/jquery-3.1.1.min.js"></script>
    <script src="../js/bootstrap.js"></script>
    <script src="../css/click.js"></script>
    
</head>
<body>
    <main>
        <nav id="menu">

            <div id="logo"><a href="../index.jsp"> T E S L A</a></div>

            <div class="clear"></div>

        </nav>








    <section class="session" id="model_y3">
  
        <div class="container">
            <div id="myCarousel" class="carousel slide" data-ride="carousel">
                <ol class="carousel-indicators">
                    <li data-target="#myCarousel" data-slide-to="1" class="active"></li>
                    <li data-target="#myCarousel" data-slide-to="2"></li>
                    <li data-target="#myCarousel" data-slide-to="3"></li>
                    <li data-target="#myCarousel" data-slide-to="4"></li>
                    <li data-target="#myCarousel" data-slide-to="5"></li>
                    <li data-target="#myCarousel" data-slide-to="6"></li>
                    <li data-target="#myCarousel" data-slide-to="7"></li>
                    <li data-target="#myCarousel" data-slide-to="8"></li>
                </ol>
                <div class="carousel-inner">
                    <div class="item active">
                        <img style="width: 100%; height: 100vh;" src="../img/x1.png">
                    </div>
                    <div class="item">
                        <img style="width: 100%; height: 100vh;" src="../img/x2.png">
                    </div>
                    <div class="item">
                        <img style="width: 100%; height: 100vh;" src="../img/x3.png">
                    </div>
                    <div class="item">
                        <img style="width: 100%; height: 100vh;" src="../img/x4.png">
                    </div>
                    <div class="item">
                        <img style="width: 100%; height: 100vh;" src="../img/x5.png">
                    </div>
                    <div class="item">
                        <img style="width: 100%; height: 100vh;" src="../img/x6.png">
                    </div>
                    <div class="item">
                        <img style="width: 100%; height: 100vh;" src="../img/x7.png">
                    </div>
                    <div class="item">
                        <img style="width: 100%; height: 100vh;" src="../img/x8.png">
                    </div>

                </div>
                <a class="left carousel-control" href="#myCarousel" data-slide="prev">
                    <span class="glyphicon glyphicon-chevron-left"></span>
                </a>
                <a class="right carousel-control" href="#myCarousel" data-slide="next">
                    <span class="glyphicon glyphicon-chevron-right"></span>
                </a>
            </div>
        </div>
    
    </section>






    <form action="">
    <section class="side_session" style=" margin-top: 4%;">
        <div>
            <div class="model">Model X</div>

            <div style="margin-top: 20%;">
                <ul class="low">
                    <li>560km</li>
                    <li>주행가능거리</li>
                </ul>
                <ul class="low">
                    <li>232km/h</li>
                    <li>최고속도</li>
                </ul>
               
            </div>

            <div class="butt">
                <p style="margin-left: 40%; font-weight: bold;">듀얼 모터</p>
                <input type="botton" value="Model X" class="button1" style="margin-top: 0%;">
                <p style="margin-left: 40%; font-weight: bold; margin-top: 10%;" >트라이 모터</p>
                <input type="botton" value="Model X Palid " class="button1" style="margin-top: 0%;">
            </div>

            <div class="model2" style="margin-top: 30%;">색 상</div>

            <ul class="color_ul">
                <li class="color_li">
                    <div>
                        <button class="radius"><img class="button_size" src="../img/Paint_Black.png" alt=""></button>
                    </div>
                </li>
                <li class="color_li">
                    <div>
                        <button class="radius"><img class="button_size" src="../img/Paint_Blue.png" alt=""></button>
                    </div>
                </li>
                <li class="color_li">
                    <div>
                        <button class="radius"><img class="button_size" src="../img/Paint_MidnightSilver.png" alt=""></button>
                    </div>
                </li>
                <li class="color_li">
                    <div>
                        <button class="radius"><img class="button_size" src="../img/Paint_Red.png" alt=""></button>
                    </div>
                </li>
                <li class="color_li">
                    <div>
                        <button class="radius"><img class="button_size" src="../img/Paint_White.png" alt=""></button>
                    </div>
                </li>
            </ul>

        </div>

    


    <div class="clear"></div>


    
        <div>
            <div class="model2" style="margin-top: 40%;">휠</div>

            <ul class="color_ul">
                <li class="color_li2">
                    <div>
                        <button class="radius"><img class="button_size2" src="../img/wheels.png" alt=""></button>
                    </div>
                </li>
                <li class="color_li2">
                    <div>
                        <button class="radius"><img class="button_size2" src="../img/wheels2.png" alt=""></button>
                    </div>
                </li>
            </ul>

        </div>




        <div>
            <div class="model2" style="margin-top: 40%;">인테리어</div>

            <ul class="color_ul">
                <li class="color_li2" style="margin-left: 10%;">
                    <div>
                        <button class="radius"><img class="button_size2" src="../img/ui_swat_int_in3pb.png" alt=""></button>
                    </div>
                </li>
                <li class="color_li2" style="margin-left: 10%;">
                    <div>
                        <button class="radius"><img class="button_size2" src="../img/ui_swat_int_white.png" alt=""></button>
                    </div>
                </li>
                <li class="color_li2"style="margin-left: 10%;">
                    <div>
                        <button class="radius"><img class="button_size2" src="../img/Interior_Cream.png" alt=""></button>
                    </div>
                </li>
            </ul>

        </div>

        </div>




    

    <div class="clear"></div>

    




        <div>
            <div class="model2" style="margin-top: 40%;">시트구성</div>


            <ul class="color_ul">
                <li class="color_li2" style="margin-left: 10%;">
                    <div>
                        <button class="button3">5</button>
                    </div>
                </li>
                <li class="color_li2" style="margin-left: 10%;">
                    <div>
                        <button class="button3">6</button>
                    </div>
                </li>
                <li class="color_li2"style="margin-left: 10%;">
                    <div>
                        <button class="button3">7</button>
                    </div>
                </li>
            </ul>


            <div class="model2" style="margin-top: 40%;">주문하기</div>
            <input type="submit" value="지금 주문하기" class="button2" style="margin-top: 10%;">



        </div>




    


    <div class="clear"></div>

    


    
        <div style="width: 70%; margin-left: 15%; margin-top: 10%;">



            <P>테슬라코리아 유한회사 | 사업자등록번호 : 524-88-00237 | 데이비드존파인스타인, 리리 | 통신판매업신고: 제2016-서울강남-02964호 | 사업자정보확인 | 호스팅제공자: Tesla Inc. | 주소 : 서울특별시 강남구 테헤란로 134, 14층 | 대표전화 : 080-617-1388 | southkorea@tesla.com</P>
          




        </div>




    </section>








    
</form>
    </main>
</body>
</html>
<!--sadfsadfasdf-->